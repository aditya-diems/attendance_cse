...
def deletteFromDailyReport(year,division,date,subject,torp,roll):
    ...
    if table_exist:
        ...
        if torp == "Theory":
            ...
            for i in roll:
                sql = "UPDATE `{}` SET `{}`=-1 WHERE roll={}".format(tableName,col[ind],i)
                dcse_cur.execute(sql)
                dcse.commit()
            ...
            for i in roll:
                ...
                dcse_cur.execute(sql)
                dcse.commit()
                sql = "UPDATE `{}` SET `{}`=-1 WHERE roll={}".format(tableName,col[indnext],i)
                dcse_cur.execute(sql)
                dcse.commit()
                sql = "UPDATE `{}` SET `prsub`='' WHERE roll={}".format(tableName,i)
                dcse_cur.execute(sql)
                dcse.commit()
...
def deleteAttendanceTheory(year,division,date,subject,batch,torp):
    ...
    if year == "BTECH":
        ...
        try:
            for i in batch:
                sql = "SELECT ROLL_NO from `{}` WHERE batch='{}'".format(year,i)
                cur.execute(sql)
                temp = cur.fetchall()
                for k in temp:
                    roll.append(k[0])        
            ...
            for i in roll:
                sql = "UPDATE `{}` SET {}=-1 WHERE roll={}".format(subject,date,i)
                btech.execute(sql)
                at_btech.commit()
        except:
